...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, expand_ratio, ConvBlock):
        ...
        if expand_ratio != 1:
            conv_params = (in_channels, hidden_dim, 1, True)
...
class ColumnFC(nn.Module):
    ...
    def __init__(self, input_dim=446, output_dim=30, dropout=0.5):
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=2048, dropout=0.1):
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2).float() *   # (d_model/2,)
                              (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)                 # 偶数维
        pe[:, 1::2] = torch.cos(position * div_term)                 # 奇数维
        pe = pe.unsqueeze(0)                                         # (1, max_len, d_model)
        self.register_buffer('pe', pe)                               # 作为buffer保存（不参与训练）
