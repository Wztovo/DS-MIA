...
class TrainerPrivate(object):
    """
        功能：
            封装客户端本地训练与差分隐私/对抗防御逻辑，同时提供评估接口。
        主要方法：
            - mixup_data: 对一个 batch 执行 MixUp 数据增强。
            - mixup_criterion: 计算 MixUp 后的加权交叉熵损失。
            - _local_update_noback: 在单个客户端上执行若干本地训练轮（不保留梯度历史），并返回更新后模型权重与平均损失。
            - test: 在给定 DataLoader 上做前向评估，返回平均损失与准确率。
            - fake_test: 对预测结果做“伪标签”再评估，用于检测模型记忆等。
        """
    def __init__(self, model, train_set, device, dp, sigma,num_classes,defense=None,klam=3,up_bound=0.65,mix_alpha=0.01):
        """
                功能：
                    初始化本地训练工具，设置模型、设备、差分隐私与防御参数等。
                参数：
                    model (nn.Module)      : 要训练的 PyTorch 模型。
                    train_set (Dataset)    : 整个训练集（后续按索引切分给各客户端）。
                    device (str/torch.device): 训练设备，如 'cuda' 或 'cpu'。
                    dp (bool)              : 是否启用差分隐私（DP）。
                    sigma (float)          : DP 噪声标准差。
                    num_classes (int)      : 分类任务类别数。
                    defense (str)          : 对抗防御方式，可选 'mix_up','instahide' 等。
                    klam (int)             : Instahide 防御的 k 参数。
                    up_bound (float)       : Instahide λ 上界。
                    mix_alpha (float)      : MixUp 的 Beta 分布 α 参数。
                返回：
                    None：实例属性设置完毕。
                """
        self.model = model # 全局共享模型
        self.device = device # 训练/评估设备
        self.tester = TesterPrivate(model, device)# 内置评估器，用于本地测试
        self.dp = dp# 差分隐私开关
        self.sigma = sigma# DP 噪声强度
        self.defense=defense # 对抗防御方式
        self.klam=klam # Instahide 参数 k
        self.up_bound=up_bound# Instahide λ 上界
        self.mix_alpha=mix_alpha# MixUp α 参数
        
        self.num_classes=num_classes# 分类数
        self.train_loader=train_set# 全量训练集（切分后用）
        self.batch_size=100# 默认本地批大小
    

    def mixup_data(self, x, y,alpha):
        """
               功能：
                   对输入 batch 执行 MixUp 数据增强，返回混合后的输入、对应的两组标签与混合系数 λ。
               参数：
                   x (Tensor)   : 原始输入张量，形状 [B, C, H, W]。
                   y (Tensor)   : 原始标签张量，形状 [B]。
                   alpha (float): Beta 分布的参数 α，决定 λ 的随机分布。
               返回：
                   mixed_x (Tensor): 按 λ 混合后的输入。
                   y_a (Tensor)    : 原始标签 y。
                   y_b (Tensor)    : 打乱后标签 y[index]。
                   lam (float)     : 从 Beta(alpha,alpha) 采样得到的混合系数。
               """
        use_cuda=True
        # print('alpha:',alpha)
